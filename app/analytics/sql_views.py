...
BILLS_PLAYS_VIEW = "bills_plays"
PLAY_FILE_PATTERN = re.compile(r"^bills_plays_(\d{4})\.parquet$")
...
def processed_play_paths(data_dir: Path = PROCESSED_DATA_DIR) -> list[Path]:
    paths = []
    for path in data_dir.glob("bills_plays_*.parquet"):
        if PLAY_FILE_PATTERN.match(path.name):
            paths.append(path)

    paths.sort(key=season_from_path)
    if not paths:
        raise AnalyticsViewError(f"No cleaned Bills play files found in {data_dir}.")
...
def initialize_analytics_views(
    ...
    create_bills_plays_view(connection, play_paths)
...
def create_bills_plays_view(
    connection: duckdb.DuckDBPyConnection,
    paths: list[Path],
) -> None:
    if not paths:
        raise AnalyticsViewError("Cannot create bills_plays view without files.")

    path_list = ", ".join(sql_string(str(path)) for path in paths)
    connection.execute(
        "CREATE OR REPLACE VIEW bills_plays AS "
        f"SELECT * FROM read_parquet([{path_list}], union_by_name=true)"
    )


def describe_bills_plays(connection: duckdb.DuckDBPyConnection) -> list[dict[str, Any]]:
    rows = connection.execute(f"DESCRIBE {BILLS_PLAYS_VIEW}").fetchall()
...
def available_seasons(connection: duckdb.DuckDBPyConnection) -> list[int]:
    rows = connection.execute(
        f"SELECT DISTINCT season FROM {BILLS_PLAYS_VIEW} ORDER BY season"
    ).fetchall()
